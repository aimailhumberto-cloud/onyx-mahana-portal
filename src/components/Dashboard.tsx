...
interface ChartData {
  ...
  leadsAbiertos: { cantidad: number; monto: number }
  mesActual: string
}
        ...
        <div className="bg-white rounded-xl shadow-sm px-4 py-3 border border-gray-100">
          <div className="flex items-center justify-between mb-1.5">
            <div>
              ...
              <p className="text-[9px] text-gray-400 uppercase tracking-wide">Ventas Directas</p>
            </div>
            <img src="/mahana-logo.jpg" alt="Mahana" className="w-10 h-10 rounded-lg object-cover shadow-sm ring-1 ring-gray-200" />
          </div>
          <p className="text-2xl font-bold text-azul-900">{data.tours_mahana.total}</p>
          <div className="flex items-center gap-3 mt-1 text-xs">
            <span className="text-gray-500">Ing: <strong className="text-green-600">{fmt(data.tours_mahana.ingresos)}</strong></span>
            <span className="text-gray-500">Gan: <strong className="text-turquoise-600">{fmt(data.tours_mahana.ganancia)}</strong></span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm px-4 py-3 border border-gray-100">
          <div className="flex items-center justify-between mb-1.5">
            <div>
              ...
              <p className="text-[9px] text-gray-400 uppercase tracking-wide">Ventas Partners</p>
            </div>
            <img src="/caracol-logo.png" alt="Caracol" className="w-10 h-10 rounded-lg object-contain bg-blue-50 p-0.5 shadow-sm" />
          </div>
          <p className="text-2xl font-bold text-azul-900">{data.ventas_partners.total}</p>
          <div className="flex items-center gap-3 mt-1 text-xs">
            <span className="text-gray-500">Ing: <strong className="text-green-600">{fmt(data.ventas_partners.ingresos)}</strong></span>
            <span className="text-gray-500">Com: <strong className="text-purple-600">{fmt(data.ventas_partners.comisiones)}</strong></span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm px-4 py-3 border border-gray-100">
          <div className="flex items-center justify-between mb-1.5">
            <div>
              ...
              <p className="text-[9px] text-gray-400 uppercase tracking-wide">Hospedaje</p>
            </div>
            <div className="flex items-center -space-x-2">
              <img src="/caracol-logo.png" alt="Caracol" className="w-9 h-9 rounded-lg object-contain bg-white p-0.5 ring-2 ring-white shadow-sm" />
              <img src="/casa-mahana-logo.png" alt="Casa Mahana" className="w-9 h-9 rounded-lg object-contain bg-white p-0.5 ring-2 ring-white shadow-sm" />
            </div>
          </div>
          <p className="text-2xl font-bold text-azul-900">{data.resumen.estadias_total}</p>
          <div className="flex items-center gap-3 mt-1 text-xs">
            <span className="text-gray-500">Pend: <strong className="text-arena-600">{data.resumen.estadias_pendientes}</strong></span>
            <span className="text-gray-500">Conf: <strong className="text-turquoise-600">{data.resumen.estadias_confirmadas}</strong></span>
            {estadiasComisiones > 0 && <span className="text-gray-500">Com: <strong className="text-purple-600">{fmt(estadiasComisiones)}</strong></span>}
          </div>
        </div>
      </div>
            ...
            <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
              ...
              <ResponsiveContainer width="100%" height={170}>
                <BarChart data={charts.ingresosPorMes} margin={{ top: 5, right: 5, left: -10, bottom: 5 }}>
                  ...
                  <Bar dataKey="ganancia" name="Ganancia" fill="#3b82f6" radius={[3, 3, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
              <div className="flex items-center justify-center gap-4 mt-1 text-[10px] text-gray-500">
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-turquoise-500 inline-block" /> Ingresos</span>
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-blue-500 inline-block" /> Ganancia</span>
              </div>
            </div>
          )}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
        ...
        <div className="divide-y divide-gray-50">
          {data.recientes && data.recientes.length > 0 ? (
            data.recientes.slice(0, 3).map((item, i) => (
              <div key={`${item.tipo}-${item.id}-${i}`} className="px-4 py-2.5 flex items-center justify-between hover:bg-gray-50/50 transition-colors">
                <div className="flex items-center gap-2.5 flex-1 min-w-0">
                  <span className={`text-xs px-2 py-0.5 rounded-md font-medium ${
                    item.tipo === 'tour' ? 'bg-turquoise-100 text-turquoise-700' : 'bg-purple-100 text-purple-700'
                  }`}>{item.tipo === 'tour' ? '🏄' : '🏨'}</span>
                  <div className="min-w-0">
                    <p className="font-medium text-azul-900 text-xs truncate">{item.cliente}</p>
                    <p className="text-[10px] text-gray-500 truncate">{item.descripcion}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 text-right shrink-0">
                  {item.monto && <span className="font-semibold text-xs text-azul-900">{fmt(item.monto)}</span>}
                  <span className="text-[9px] text-gray-400">{item.fecha}</span>
                  {item.fuente === 'openclaw' && <Bot className="w-3 h-3 text-purple-400" />}
                  {item.fuente === 'manual' && <User className="w-3 h-3 text-gray-400" />}
                </div>
              </div>
            ))
          ) : (
            <div className="px-4 py-6 text-center text-gray-500 text-sm">No hay actividad reciente</div>
          )}
        </div>
      </div>
